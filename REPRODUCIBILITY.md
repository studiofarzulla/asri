...
|---|---------|-------------|---------------------|--------|
| 1 | `python scripts/event_study_hac.py` | HAC t(L=20): Terra 1.72 (p=0.086, n.s.), Celsius 3.49, FTX 3.28, SVB 3.86; event study **inconclusive**, not counted toward detection (asymptotic-normal ref would clear 3/4, but that ref is invalid; on the valid fixed-b ref only SVB clears, **1/4**) | Terra **1.72** (p=0.086), Celsius **3.49** (p=0.001), FTX **3.28** (p=0.001), SVB **3.86** (p<0.001); the p<0.05 flags use the asymptotic-normal ref (Celsius/FTX/SVB); on the fixed-b ref only SVB clears (**1/4**), and the placebo test shows even that is not crisis-specific -- paper treats the event study as inconclusive | PASS |
...
| 4 | `python scripts/generate_detection_table.py` | peaks Terra 48.7 / Celsius 71.4 / **FTX 84.7** / SVB 68.7; fixed τ=50 → **3/4** (Terra<50); event study inconclusive (not counted; Event-Sig column = 0/4 in the detection matrix); walk-forward OOS **4/4** | identical: FTX peak **84.7**, τ=50 recall **3/4** (75%), event-sig not counted (inconclusive), WF-OOS **4/4** (100%) | PASS |
